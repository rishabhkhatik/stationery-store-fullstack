      ...
      <header style={{ background: '#fff', boxShadow: '0 2px 12px rgba(0,0,0,0.08)', position: 'sticky', top: 0, zIndex: 100 }}>
        <div className="container" style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 16px' }}>
          ...
          <Link to="/" style={{ flexShrink: 0 }}>
            {siteConfig.logo ? (
              <img src={siteConfig.logo} alt={siteConfig.storeName} style={{ height: 48, objectFit: 'contain' }} />
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <span style={{ fontSize: 20, fontWeight: 700, color: 'var(--primary)', lineHeight: 1.1 }}>{siteConfig.storeName}</span>
                <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{siteConfig.tagline}</span>
              </div>
            )}
          </Link>

          {/* Search bar */}
          <div ref={searchRef} style={{ flex: 1, position: 'relative', maxWidth: 600 }}>
            <div style={{ display: 'flex', alignItems: 'center', background: '#f5f5f5', borderRadius: 30, padding: '8px 16px', gap: 8 }}>
              ...
              />
              {searchQuery && <X size={14} style={{ cursor: 'pointer', color: 'var(--text-muted)' }} onClick={() => { setSearchQuery(''); setSearchResults([]) }} />}
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            ...
            {/* Mobile menu toggle */}
            <button onClick={() => setMobileMenu(!mobileMenu)} style={{ display: 'none', padding: 8 }} className="mobile-menu-btn">
          ...
          <div style={{ display: 'flex', gap: 0, padding: '0 16px', whiteSpace: 'nowrap' }}>
            {['Pencil', 'Notebook', 'Stapler', 'Lunch Box', 'Sippers & Bottles', 'Multipurpose Containers', 'Giftset', 'Slime', 'Toys'].map(cat => (
              <Link key={cat} to={`/categories?cat=${cat.toLowerCase().replace(/\s+/g, '-').replace('&', '')}`}
                style={{ display: 'inline-block', padding: '10px 14px', fontSize: 13, fontWeight: 500, color: 'var(--text-muted)', transition: 'color 0.2s', borderBottom: '2px solid transparent', whiteSpace: 'nowrap' }}
            ...
            <Link to="/categories"
              style={{ display: 'inline-block', padding: '10px 14px', fontSize: 13, fontWeight: 600, color: 'var(--primary)', whiteSpace: 'nowrap' }}>
              All Categories →
            </Link>
          </div>
        </div>
